...
public class Folder
{
                      ...
                      throws FileException
    {
        ...
        if (tempStr.length() == 0)
        {
            throw new FileException(LogMessages.NAME_NOT_SPECIFIED);
        }
        setName(tempStr);
        ...
        if (tempStr.length() == 0)
        {
            throw new FileException(LogMessages.ACTUAL_LOCATION_NOT_SPECIFIED);
        }
        ...
        if (!dir.exists())
        {
            if (!dir.mkdirs())
            {
                throw new FileException(LogMessages.UNABLE_TO_CREATE_INPUT_FOLDER,dir);
            }
        }
        else
        {
            if (!dir.isDirectory())
            {
                throw new FileException(LogMessages.INPUT_DIR_IS_NOT_A_DIRECTORY,dir);
            }
        }

        if (!dir.canRead())
        {
            throw new FileException(LogMessages.INPUT_DIR_NOT_REACHABLE,dir);
        }

        if (!dir.canWrite())
        {
            throw new FileException(LogMessages.INPUT_DIR_NOT_WRITABLE,dir);
        }
        setDirectory(dir);
        ...
        if (tempStr.length() == 0)
        {
            throw new FileException(LogMessages.TRACK_TIME_PARAM_NOT_SPECIFIED);
        }
        ...
        {
            throw new FileException(e,LogMessages.INVALID_TRACK_TIME, tempStr);
        }
        ...
        if (triggerNode == 0)
        {
            throw new FileException(LogMessages.TRIGGER_NODE_UNDEFINED);
        }
        ...
        try
        {
            ...
            {
                throw new FileException(LogMessages.METHODNAME_NOT_SPECIFIED);
            }
            namespace = xpProp.getStringValue("namespace", "");

            if (namespace.length() == 0)
            {
                throw new FileException(LogMessages.NAMESPACE_NOT_SPECIFIED);
            }
        ...
        {
        	throw new FileException(e,LogMessages.TRIGGER_NODE_UNDEFINED);
        }
    }
                                        ...
                                        throws FileException
    {
        ...
        {
            throw new FileException(e,LogMessages.UNABLE_TO_LOAD_ERROR_HANDLER_CLASS,className);
        }
    }
                                ...
                                throws FileException
    {
        ...
        try
        {
            ...
            {
                throw new FileException(LogMessages.INVALID_FILTER_TYPE,type);
            }
        ...
        {
            throw new FileException(LogMessages.INVALID_FILE_FILTER, pattern);
        }
    }
