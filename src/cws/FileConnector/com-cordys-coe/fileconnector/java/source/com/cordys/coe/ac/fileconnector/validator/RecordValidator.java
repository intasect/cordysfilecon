...
public class RecordValidator {
    ...
    public int parseAndValidateRecord(String sFileType, CharSequence csInput, int iInputPos,
        ...
        if (ftFileType == null) {
            throw new ValidationException(LogMessages.INVALID_FILE_TYPE,sFileType);
        }

        if (!ftFileType.bAllowEmptyFiles && csInput.length() == 0) {
            throw new ValidationException(LogMessages.FILE_EMPTY);
        }
        ...
        while (iInputPos < csInput.length()) {
            ...
            if (!ftFileType.pmPartialRecordMatcher.isCompleteMatch(sbReadRecordNames)) {
            	if( !bContinueOnError )
            	{
	                ...
	                throw new ValidationException(LogMessages.AT_LINE_NO_MATCHING_RECORDS_FOUND,
	                                              iCurrentRecordNumber,
	                                              sbReadRecordNames);
...
//    	Node.appendToChildren(iErrorRecordNode, iResultNode);
                     ...
                     throws ValidationException
    {
        ...
        if ((rtRecord.iRecordPatternGroup < 0)
            ...
            throw new ValidationException(LogMessages.AT_LINE_GROUP_INDEX_NOT_FOUND,
                                          iCurrentRecordNumber ,
                                          rtRecord.iRecordPatternGroup,
                                          rtRecord.sRecordName);
        }
